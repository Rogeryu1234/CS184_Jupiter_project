...
Ray Camera::generate_ray(double x, double y) const {
  ...
  // Instantiate virtual camera sensor location in camera space.
  Vector3D bottomLeft(-tan(0.5 * radians(hFov)), -tan(0.5 * radians(vFov)), (double)-1);
  Vector3D topRight(tan(0.5 * radians(hFov)), tan(0.5 * radians(vFov)), (double)-1);

  // Width and length of virtual camera sensor.
  double delta_x = topRight.x - bottomLeft.x;
  double delta_y = topRight.y - bottomLeft.y;

  // Create ray in camera space.
  Vector3D imageCameraSpace(bottomLeft.x + x * delta_x, bottomLeft.y + y * delta_y, (double)-1);

  // Transform it into world space.
  Vector3D imageWorldSpace = c2w * imageCameraSpace;
  imageCameraSpace.normalize();

  Ray rayWorldSpace = Ray(pos, imageWorldSpace);
  rayWorldSpace.max_t = fClip;
  rayWorldSpace.min_t = nClip;
